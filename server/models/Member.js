...
// models/Member.js - COMPLETE UPDATED FILE
...
// // ========== INDEXES ==========
// // Remove duplicate indexes - define each only once

// // Text index for search
// memberSchema.index({ name: 'text', surname: 'text', memberNumber: 'text', rollNumber: 'text', vanshaGenerationNumber: 'text', phone: 'text', email: 'text', citizenshipNumber: 'text' });

// // Single-field indexes - REMOVED duplicate index declarations
// // These were previously defined with index: true in schema
// memberSchema.index({ memberNumber: 1 }, { unique: true, sparse: true });
// memberSchema.index({ rollNumber: 1 }, { unique: true, sparse: true });
...
const memberSchema = new mongoose.Schema({
  ...
  dod: {
    type: Date,
    default: null, // ⭐ Nullable Date of Death
  },
...
memberSchema.pre('save', async function() {
  ...
  if (this.isNew && !this.memberNumber) {
    ...
    this.memberNumber = `MEM-${String(counter.seq).padStart(6, '0')}`;
  }
});
...
//   // ⭐ UPDATED: Allow any Nepali relationship value
//   relationship: {
//     type: String,
//     trim: true,
//     default: 'सदस्य',
...
// // Single-field indexes - NO unique on vanshaGenerationNumber
// memberSchema.index({ memberNumber: 1 }, { unique: true, sparse: true });
// memberSchema.index({ rollNumber: 1 }, { unique: true, sparse: true });
// memberSchema.index({ phone: 1 }, { sparse: true });
// memberSchema.index({ email: 1 }, { sparse: true });
// memberSchema.index({ vanshaGenerationNumber: 1 }); // ⭐ NOT UNIQUE
// memberSchema.index({ generation: 1 });
// memberSchema.index({ isAlive: 1 });
// memberSchema.index({ gender: 1 });
// memberSchema.index({ family: 1 });
